...
Реализует рекомендации Codex по управлению токенами
"""
import re
...
class ContextOptimizer:
    """Оптимизирует контекст согласно рекомендациям Codex"""

    def __init__(self):
        # Рекомендации Codex
        self.max_context_tokens = 2000  # Максимальный контекст
    ...
    def optimize_context(self, documents: List[Dict[str, Any]], query: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            Оптимизированный список документов
        """
        if not documents:
            return []
    ...
    def _optimize_text(self, text: str, max_tokens: int) -> str:
        """Оптимизирует текст согласно рекомендациям Codex"""
        if not text:
            return ""
