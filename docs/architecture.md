...
### 🎉 Текущая версия: v4.5.0 (2025-11-26)
...
- **Query Type Classification** (v4.5.0): автоматическая классификация типа запроса для адаптивной стратегии retrieval:
  - `FACTUAL` — фактические вопросы ("Что такое X?") → k=15, rerank=3
  - `PROCEDURAL` — инструкции ("Как настроить X?") → k=25, rerank=6, auto_merge=ON
  - `COMPARATIVE` — сравнения ("Чем X отличается от Y?") → k=20, rerank=5
  - `TROUBLESHOOTING` — диагностика ("Ошибка X") → k=30, rerank=7, auto_merge=ON
  - `LIST` — перечисления ("Какие есть X?") → k=10, rerank=2
  - `EXPLORATORY` — общие вопросы → k=20, rerank=5, auto_merge=ON
...
Query Processing (process_query)
   ...
   - ⭐ Query Type Classification (v4.5.0)
   - ⭐ Retrieval Strategy selection (k, rerank_top_n, auto_merge)
   ↓
...
Hybrid Search (hybrid_search)
   - dense + sparse → RRF
   - ⭐ Adaptive k based on query_type (v4.5.0)
...
Rerank (bge-reranker-v2-m3)
   - ⭐ Adaptive top_n based on query_type (v4.5.0)
   ↓
Auto-Merge Neighbors (conditional based on strategy)
   ↓
Context Optimizer (optimize_context)
   - ⭐ Long Context Reorder (v4.5.0)
   ↓
...
**Context Optimizer**:
- **Long Context Reorder** (v4.5.0): переупорядочивание документов для оптимального внимания LLM
  - Mitigation эффекта "Lost in the Middle" (Liu et al., 2023)
  - Лучшие документы размещаются в начале и конце контекста
  - Автоматически применяется для контекста > 2 документов
...
16. **Query Type Classification** (v4.5.0): Адаптивная стратегия retrieval на основе типа запроса → оптимальный k/rerank_top_n для каждого типа
17. **Long Context Reorder** (v4.5.0): Переупорядочивание документов для mitigation эффекта "Lost in the Middle" → улучшенное использование контекста LLM
