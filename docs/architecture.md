...
### 🎉 Обновления архитектуры (v4.1.0)

**Критические исправления гибридного поиска завершены!** Система полностью готова к production использованию:
...
6) **Критические исправления гибридного поиска (v4.1.0)**
   ...
   **📦 Utils** (`ingestion/utils/`):
   ...
   - Qdrant Writer:
     ...
     - Инкрементальность: сравнение хэшей контента/updated_at; upsert измененных чанков, удаление убывших.
...
7) Storage (Qdrant)
   - Коллекция: `chatcenter_docs`
     ...
     - Payload (метаданные):
       ...
       - `doc_id`, `heading_path` - новые индексы для удобной фильтрации.
   - Индекс HNSW:
...
### Ключевые изменения v4.1.0

**🚨 Критические исправления:**
- Исправлена схема коллекции для sparse векторов
- Sparse векторы вынесены в отдельный параметр sparse_vectors
- Гибридный поиск dense + sparse работает корректно

**✅ Надежность и стабильность:**
- Ретраи для upsert с экспоненциальным backoff
- Валидация размерности dense векторов
- Защита от numpy типов в сериализации
- Автоматическое создание коллекций с правильной схемой

**✅ Расширенные возможности:**
- Дополнительные индексы: doc_id, heading_path
- Улучшенный sparse конвертер с поддержкой разных форматов
- Фильтрация по категориям в retrieval
- Увеличенный recall в RRF (k * 2)

**✅ Улучшенное логирование:**
- Контекст для отладки проблемных документов
- Статистика нулевых dense векторов
- Подробные логи создания коллекций
