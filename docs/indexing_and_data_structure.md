...
#### Архитектура UniversalChunker

- Вся логика разбиения Markdown перенесена в отдельный state-machine `_MarkdownBlockParser`, который гарантирует корректную обработку admonitions, списков и fenced‑кода без копипасты в основном классе.
- Формирование итоговых объектов `Chunk` выполняет `_ChunkAssembler`. Он отвечает за постановку заголовков, расчёт индексов и перенос overlap-блоков, поэтому метод `chunk()` теперь описывает только общий пайплайн (blockify → oversize policy → semantic packing → overlap → сборка).
- Благодаря этим вспомогательным слоям поддержка новых типов блоков и модификация правил overlap не требуют изменения монолитного метода на сотню строк.
