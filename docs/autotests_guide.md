...
├── test_ragas_quality.py           # RAGAS качество
├── test_optimized_architecture.py   # Архитектура
├── test_data_validation.py         # Валидация данных
├── test_crawlers.py                # Краулеры
├── test_universal_loader.py        # Универсальный загрузчик
└── test_*.py                       # Другие тесты

scripts/
├── indexer.py                      # Production индексатор
├── init_qdrant.py                  # Инициализация БД
├── manage_cache.py                 # Управление кешем
├── run_tests.py                    # Запуск тестов
└── *.py                           # Production утилиты
