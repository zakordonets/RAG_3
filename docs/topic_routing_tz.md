...
### 3.4. Интеграция роутера (архитектурный уровень)

- В сервисе, который обрабатывает пользовательский запрос (до `hybrid_search`):
  - вызывать `route_query(query, user_metadata)`;
  - по результату:
    - при высокой уверенности:
      - построить Qdrant‑фильтр по полям `domain/section/platform/role` для `primary_theme`;
    - при нескольких темах:
      - либо строить фильтр по нескольким `Theme` (например, `domain IN {…}`),
      - либо выставлять `requires_disambiguation=True` и не запускать поиск до уточнения.
  - передавать `routing_result` в `hybrid_search` через `context`.
- Сам `hybrid_search` **не вызывает** `route_query` и не знает, как построены тематики — он лишь использует переданный `routing_result` и уже сформированный фильтр для Qdrant.
...
- Если `routing_result.primary_theme` определена с высокой уверенностью и `requires_disambiguation=False`:
  - сервис поиска строит Qdrant‑фильтр только по этой теме (`Theme`);
  - `hybrid_search` работает уже внутри узкого тематического множества.
- Это покрывает кейсы вроде запроса «как подключить SDK в Android приложении?»:
  - темы из `chatcenter_user_docs` и iOS/Web‑SDK не должны попадать в выдачу.

### 4.2. Мягкий тематический буст (несколько тем)

- Если `routing_result.themes` содержит несколько тем и `requires_disambiguation=False`:
  - фильтр для Qdrant может включать несколько тематик (например, `domain IN {"chatcenter_user_docs", "sdk_docs"}`);
  - `routing_result` передаётся в модуль буста, который:
    - повышает score документов, принадлежащих более вероятным темам (см. `_apply_theme_boost_from_routing`);
    - оставляет менее вероятные темы внизу списка.

Это полезно, если вопрос **осознанно** затрагивает несколько тематик (например, «что должен делать оператор и что администратор для настройки X»).

### 4.3. Уточняющие вопросы (requires_disambiguation=True)

- Если `route_query` возвращает `requires_disambiguation=True`:
  - поиск не запускается, пока пользователь не уточнит, какую тематику имел в виду;
  - LLM формирует уточняющий вопрос на основе `routing_result.themes` и их `display_name`, например:
    - «Вы спрашиваете про настройки в SDK для Android или про настройки в веб-интерфейсе (АРМ администратора)?»
  - пользователь выбирает одну или несколько тематик (в UI или текстом);
  - после выбора тем:
    - сервис поиска строит фильтр по выбранным темам;
    - вызывает `hybrid_search` с соответствующим `routing_result`.
